...
def _default_file_path() -> str:
    ...
    # The connector's module name can be inferred by looking at the modules loaded 
    # and look for the one starting with source_
    for k in sys.modules:
        if "source_" in k and "airbyte_cdk" not in k:
            module = k.split(".")[0]
            return f"./{module}/schemas/{{{{parameters['name']}}}}.json"

    # If we are not in a source_ module, the most likely scenario is we're processing
    # a manifest from the connector builder server which does not require a json schema 
    # to be defined.
